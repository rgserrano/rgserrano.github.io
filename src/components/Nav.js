...
const Nav = () => {
  ...
  return (
    <div className="fixed top-8 right-8 overflow-hidden flex flex-col items-center">
      <button className="relative z-50 rounded-lg mb-4" onClick={() => setVisible(!visible)} >
        <img src={menuIcon} alt="The dropdown menu icon" className="w-16 h-16" />
      </button>
      <nav className={`relative z-0 flex flex-col items-center gap-4 transform transition duration-500 ease-in-out ${visible ? "translate-y-0" : "-translate-y-96"}`}>
